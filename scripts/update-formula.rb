...
# Update script for libreoffice-rest-api Homebrew formula
...
class FormulaUpdater
  GITHUB_API = 'https://api.github.com/repos/funnyzak/libreoffice-rest-api/releases/latest'
  FORMULA_PATH = File.join(__dir__, '..', 'Formula', 'libreoffice-rest-api.rb')
  ...
  def run
    puts '🚀 Starting libreoffice-rest-api formula update process...'
  ...
  def update_formula
    ...
    @formula_content = @formula_content.gsub(
      /url "[^"]*libreoffice-rest-api-darwin-amd64[^"]*"/,
  ...
  def validate_formula
    ...
    Tempfile.create(['libreoffice-rest-api_test', '.rb']) do |temp_file|
