...
# Test script for homebrew-libreoffice-rest-api tap
...
if [ ! -f "Formula/libreoffice-rest-api.rb" ]; then
    print_error "Formula/libreoffice-rest-api.rb not found. Please run this script from the repository root."
    exit 1
fi

print_status "Starting homebrew-libreoffice-rest-api test suite..."

# Test 1: Check formula syntax
print_status "Testing formula syntax..."
if ruby -c Formula/libreoffice-rest-api.rb > /dev/null 2>&1; then
...
if command -v curl &> /dev/null; then
    ...
    INTEL_URL=$(grep -A1 'on_intel do' Formula/libreoffice-rest-api.rb | grep 'url "' | sed 's/.*url "\(.*\)".*/\1/')
    ARM_URL=$(grep -A1 'on_arm do' Formula/libreoffice-rest-api.rb | grep 'url "' | sed 's/.*url "\(.*\)".*/\1/')
    INTEL_SHA=$(grep -A2 'on_intel do' Formula/libreoffice-rest-api.rb | grep 'sha256 "' | sed 's/.*sha256 "\(.*\)".*/\1/')
    ARM_SHA=$(grep -A2 'on_arm do' Formula/libreoffice-rest-api.rb | grep 'sha256 "' | sed 's/.*sha256 "\(.*\)".*/\1/')
...
required_files=(
    "Formula/libreoffice-rest-api.rb"
...
if command -v brew &> /dev/null && [ "$1" = "--full" ]; then
    ...
    if brew tap funnyzak/libreoffice-rest-api-temp "$(pwd)" 2>/dev/null; then
        print_success "Temporary tap created"

        # Try to install (this might fail if libreoffice-rest-api is already installed)
        if brew install libreoffice-rest-api-temp 2>/dev/null || true; then
            print_success "Installation test passed"

            # Test the installation
            if command -v libreoffice-rest-api &> /dev/null; then
                VERSION=$(libreoffice-rest-api --version 2>/dev/null || echo "version check failed")
                print_success "libreoffice-rest-api is working: $VERSION"

                # Cleanup
                brew uninstall libreoffice-rest-api-temp 2>/dev/null || true
            else
                print_warning "libreoffice-rest-api command not found after installation"
            fi
        else
        ...
        # Cleanup tap
        brew untap funnyzak/libreoffice-rest-api-temp 2>/dev/null || true
    else
...
print_status ""
print_status "To install libreoffice-rest-api using this tap, run:"
print_status "  brew tap funnyzak/libreoffice-rest-api <repository-url>"
print_status "  brew install libreoffice-rest-api"
print_status ""
